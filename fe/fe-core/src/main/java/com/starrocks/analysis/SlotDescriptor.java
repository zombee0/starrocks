...
public class SlotDescriptor {
    ...
    public void setUsedSubfieldPosGroup(List<List<Integer>> usedSubfieldPosGroup) {
        ...
        // Preconditions.checkArgument(type != originType);
        if (usedSubfieldPosGroup.size() == 0) {
            type.selectAll();
            return;
        }

        for (List<Integer> usedSubfieldPos : usedSubfieldPosGroup) {
            if (usedSubfieldPos.size() == 0) {
                type.selectAll();
                return;
            }
            Type tmpType = type;
    ...
    public TSlotDescriptor toThrift() {
        if (originType != null && !originType.isStructType() && !originType.isMapType() && !originType.isArrayType()) {
