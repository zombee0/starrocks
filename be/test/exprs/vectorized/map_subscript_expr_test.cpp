...
std::unique_ptr<Expr> create_map_subscript_expr(const TTypeDesc& type) {
    TExprNode node;
    node.__set_node_type(TExprNodeType::MAP_SUBSCRIPT_EXPR);
    node.__set_is_nullable(true);
    node.__set_type(type);
    node.__set_num_children(0);

    auto* expr = MapSubscriptExprFactory::from_thrift(node);
    return std::unique_ptr<Expr>(expr);
}

std::unique_ptr<Expr> create_map_subscript_expr(const TypeDescriptor& type) {
    return create_map_subscript_expr(type.to_thrift());
}

} // anonymous namespace

class MapSubscriptExprTest : public ::testing::Test {
protected:
    void SetUp() override {}
...
// NOLINTNEXTLINE
TEST_F(MapSubscriptExprTest, test_map_int_int) {
    ...
    //   NULL
    {
        std::unique_ptr<Expr> expr = create_map_subscript_expr(type_int);
...
// NOLINTNEXTLINE
TEST_F(MapSubscriptExprTest, test_map_varchar_int) {
