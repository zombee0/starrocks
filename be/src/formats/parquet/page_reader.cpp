...
StatusOr<Slice> PageReader::_read_and_decompress_internal(bool need_fill_buf) {
    ...
    auto lazy_slice = std::make_unique<LazySlice>(reinterpret_cast<char *>(_uncompressed_buf->data()), uncompressed_size, std::move(decompress_filler));
    RETURN_IF_ERROR(lazy_slice->try_to_trigger_fill(0, uncompressed_size));
